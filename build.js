...
function updateKillingFloorIni(allMutators, currentMutator = null) {
  ...
  for (let line of iniContent) {
    ...
    if (inEditorSection) {
      // Check if we're entering an AUTO-GENERATED section
      if (line.match(/^;\s*<!--\s*AUTO-GENERATED\s*-->/)) {
        inAutoGeneratedSection = true;
        continue; // Skip the opening comment
      }

      // Check if we're exiting an AUTO-GENERATED section
      if (inAutoGeneratedSection) {
        if (line.match(/^;\s*-->/)) {
          inAutoGeneratedSection = false;
          continue; // Skip the closing comment
        }
        // Skip all lines inside AUTO-GENERATED section
        continue;
      }

      // Skip other comment lines (only when not in AUTO-GENERATED section)
      if (
        line.match(/^;\s*</) ||
      ...
      if (match) {
        ...
        // Skip packages that are in allMutators (they will be added in AUTO-GENERATED section)
